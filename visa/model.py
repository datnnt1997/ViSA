...
class ABSAModel(RobertaForTokenClassification):
    def __init__(self, config, **kwargs):
        ...
        self.layer_norm = nn.LayerNorm(config.num_alabels + config.hidden_size)
    ...
    def forward(self,
        ...
        s_feats = torch.cat([valid_seq_output, a_logits], dim=-1)
        s_feats = self.activation(s_feats)
        s_feats = self.layer_norm(s_feats)
        s_logits = self.polarity_transformation(s_feats)
        # s_feats = self.activation(s_feats)
        # s_feats = self.layer_norm(s_feats)
