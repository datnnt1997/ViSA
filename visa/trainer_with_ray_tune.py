...
def train(config, args=None):
    ...
    model_config = ViSA_CONFIG_ARCHIVE_MAP[args.model_arch].from_pretrained(args.model_name_or_path,
                                                                      num_aspect_labels=len(aspect_label_map),
                                                                      num_polarity_labels=len(polarity_label_map))
    model = ViSA_MODEL_ARCHIVE_MAP[args.model_arch].from_pretrained(args.model_name_or_path, config=model_config)
    model.to(device)
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in encoder_param_optimizer if not any(nd in n for nd in no_decay)],
         'lr': config['learning_rate'], 'weight_decay': args.weight_decay},
        ...
        {'params': [p for n, p in task_param_optimizer if not any(nd in n for nd in no_decay)],
         'lr': config['classifier_learning_rate'], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in task_param_optimizer if any(nd in n for nd in no_decay)],
         'lr': config['classifier_learning_rate'], 'weight_decay': 0.0}
    ]
    optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=config['learning_rate'], eps=args.adam_epsilon)
    ...
    for epoch in range(int(args.epochs)):
        ...
        tune.report(loss=eval_loss,
                    aspect_sentiment_f1=overall_scores['macro'][-1])
...
def main():
    ...
    config = {
        "learning_rate": tune.loguniform(1e-4, 2e-5),
        "classifier_learning_rate": tune.loguniform(3e-5, 3e-1),
    ...
    reporter = CLIReporter(
        ...
        metric_columns=["loss", "aspect_sentiment_f1", "training_iteration"])
    result = tune.run(
    ...
    best_trial = result.get_best_trial("aspect_sentiment_f1", "max", "last")
    LOGGER.info("Best trial config: {}".format(best_trial.config))
    LOGGER.info("Best trial final validation loss: {}".format(
        best_trial.last_result["loss"]))
    LOGGER.info("Best trial final validation f1: {}".format(
        best_trial.last_result["aspect_sentiment_f1"]))
